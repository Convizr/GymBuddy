...
export const TimePickerExtension = {
    ...
    render: ({ trace, element }) => {
        ...
        // Use the array directly from the payload if it exists and is in the correct format
        let availableTimes = Array.isArray(trace.payload.availableTimes) ? trace.payload.availableTimes : [];

        // Check if the array is empty or not properly passed
        if (availableTimes.length === 0) {
            console.error('No available times provided or incorrect format:', trace.payload.availableTimes);
            return; // Exit if no valid time slots are available
        }
