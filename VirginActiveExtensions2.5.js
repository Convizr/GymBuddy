...
export const TimePickerExtension = {
    ...
    render: ({ trace, element }) => {
        ...
        // Handling the input string directly as it comes in a plain string format
        if (typeof trace.payload.availableTimes === 'string') {
            // Attempting to split the string directly by commas
            availableTimes = trace.payload.availableTimes.split(',');
        } else {
            console.error('Available times data is not a string:', trace.payload.availableTimes);
            return; // Exit if the format is not as expected
        }

        if (!availableTimes.length) {
            console.error('No available times parsed:', availableTimes);
