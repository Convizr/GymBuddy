...
export const TimePickerExtension = {
    ...
    render: ({ trace, element }) => {
        console.log('Trace payload:', JSON.stringify(trace.payload)); // Log the complete payload to see what it contains

        if (typeof trace.payload !== 'object' || typeof trace.payload.availableTimes !== 'string') {
            console.error('Available times data is incorrect or undefined:', trace.payload);
            return; // Exit if the payload format is not correct
        }

        let availableTimes = trace.payload.availableTimes.split(',').map(time => time.trim()); // Split and trim times

        if (!availableTimes.length) {
            console.error('No available times parsed from the payload:', availableTimes);
            return; // Exit if no times are parsed
        }
