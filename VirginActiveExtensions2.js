...
export const TimePickerExtension = {
    ...
    render: ({ trace, element }) => {
        ...
        // Initialize availableTimes from the payload, expecting a comma-separated string
        let availableTimes = [];
        ...
        if (typeof trace.payload.availableTimes === 'string') {
            ...
            console.error('availableTimes is not a string:', trace.payload.availableTimes);
            return; // Exit the render function if the format is not as expected
        }

        // Ensure we have an array after processing
        if (!Array.isArray(availableTimes) || availableTimes.length === 0) {
            console.error('No available times slots provided:', availableTimes);
