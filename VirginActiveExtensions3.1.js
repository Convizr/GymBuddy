...
export const TimePickerExtension = {
    ...
    render: ({ trace, element }) => {
        ...
        // Handle various formats of availableTimes
        if (typeof trace.payload.availableTimes === 'string') {
            try {
                // Attempt to parse it as JSON in case it's a JSON string array
                availableTimes = JSON.parse(trace.payload.availableTimes);
            } catch (error) {
                // If JSON parsing fails, try splitting as a regular string
                console.log('Trying to split string directly due to error:', error.message);
            ...
            return; // Exit the render function if the format is incorrect
        }

        // Ensure that we are working with an array after all attempts
        if (!Array.isArray(availableTimes) || availableTimes.length === 0) {
            console.error('availableTimes is not an array or is empty after processing:', availableTimes);
            return; // Handle error or fallback scenario
        }
